...
export function CalendarTile({ className }: { className?: string }) {
  ...
  return (
    <div className={cx("relative w-[8.5rem] select-none pt-2", className)} role="img" aria-label={full}>
      ...
      <div className="relative overflow-hidden rounded-2xl bg-white text-center shadow-[0_0.9rem_1.8rem_-0.6rem_rgb(0_0_0/0.5)]">
        {/* month band */}
        <div className="bg-lime px-2 pb-1.5 pt-3 text-deep">
          <div className="font-display text-[13px] font-extrabold uppercase leading-none tracking-[0.14em]">{month || " "}</div>
        ...
        <div className="px-2 pb-2.5 pt-1.5">
          <div className="font-display text-[3.4rem] font-extrabold leading-none tabular-nums tracking-tight text-ink">{d?.getDate() ?? " "}</div>
          <div className="mt-1 text-[11.5px] font-bold uppercase tracking-[0.16em] text-aqua-deep">{weekday || " "}</div>
          <div className="mx-auto mt-2 h-px w-3/4 bg-line" />
          <div className="mt-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-muted">{d ? `Week ${week} · Day ${dayOfYear}` : " "}</div>
        </div>
      </div>

      {/* binder rings */}
      <div className="absolute inset-x-0 top-0 flex justify-center gap-12">
