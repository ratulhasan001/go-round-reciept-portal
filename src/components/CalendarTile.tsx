...
export const CALENDAR_H = "9.5rem";
...
export function CalendarTile({ className }: { className?: string }) {
  ...
  return (
    <div className={cx("relative w-[8.5rem] shrink-0 select-none pt-2", className)} role="img" aria-label={full}>
      ...
      <div className="relative flex flex-col overflow-hidden rounded-2xl bg-white text-center shadow-[0_0.9rem_1.8rem_-0.6rem_rgb(0_0_0/0.5)]" style={{ height: CALENDAR_H }}>
        ...
        <div className="flex flex-1 flex-col justify-center px-2">
          <div className="font-display text-[3rem] font-extrabold leading-none tabular-nums tracking-tight text-ink">{d?.getDate() ?? " "}</div>
          <div className="mt-0.5 text-[11.5px] font-bold uppercase tracking-[0.16em] text-aqua-deep">{weekday || " "}</div>
