...
export function FlipClock({
  ...
  size?: number | string;
  seconds?: boolean;
  /** e.g. "max-sm:hidden" to drop the seconds card on narrow screens */
  ...
  return (
    <div className={cx("inline-flex items-center gap-[0.18em]", className)} style={{ fontSize: size }} role="timer" aria-label={now?.toLocaleTimeString("en-US") ?? ""}>
      ...
      {seconds && <Card value={now ? pad(now.getSeconds()) : ""} className={secondsClassName} />}
    </div>
  );
}

function Card({ value, label, className }: { value: string; label?: string; className?: string }) {
  ...
  return (
    <div className={cx("relative h-[2.3em] w-[2.55em] rounded-[0.24em] bg-[#0b241e] shadow-[0_0.2em_0.5em_rgb(0_0_0/0.45)] ring-1 ring-lime/25 [perspective:16em]", className)}>
